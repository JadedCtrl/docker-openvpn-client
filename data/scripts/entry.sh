...
case "$KILL_SWITCH" in
    'iptables')
        ...
        if [[ $SUBNETS ]]; then
            for subnet in ${SUBNETS//,/ }; do
                ip route add "$subnet" via "$default_gateway" dev "$TAP"
        ...
        while IFS= read -r line; do
            ...
            if [[ $address =~ $ip_regex ]]; then
                iptables -A OUTPUT -o "$TAP" -d "$address" -p "$protocol" --dport "$port" -j ACCEPT
            else
                for ip in $(dig -4 +short "$address"); do
                    iptables -A OUTPUT -o "$TAP" -d "$ip" -p "$protocol" --dport "$port" -j ACCEPT
    ...
    'nftables')
        ...
        while IFS= read -r line; do
            ...
            if [[ $address =~ $ip_regex ]]; then
                printf '%s\n' \
                    "add rule inet killswitch outgoing oifname "$TAP" ip daddr $address $protocol dport $port accept" >> $nftables_config_file
            else
                for ip in $(dig -4 +short "$address"); do
                    printf '%s\n' \
                        "add rule inet killswitch outgoing oifname "$TAP" ip daddr $ip $protocol dport $port accept" >> $nftables_config_file
                    printf "%s %s\n" "$ip" "$address" >> /etc/hosts
                done
            fi
        done <<< "$remotes"
